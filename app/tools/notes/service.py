...
class NotesService:
    ...
    async def search_notes(self, input: SearchNotesInput) -> SearchNotesOutput:
        async with get_db(self.db_path) as db:
            ...
            if input.tag:
                query = "SELECT * FROM notes WHERE content LIKE ? AND tags LIKE ? ORDER BY created_at DESC LIMIT 50"
                params = (f"%{input.query}%", f"%\"{input.tag}\"%")
            else:
                query = "SELECT * FROM notes WHERE content LIKE ? ORDER BY created_at DESC LIMIT 50"
